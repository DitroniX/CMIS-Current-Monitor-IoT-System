/*
  ...
  Simplified Board Bring Up Test - DC Current Sensor Calibrate, with Averaging Enabled
...
const int AverageDelay = 4;      // Average Multi-Sample Delay.
...
// **************** INPUTS ****************
#define DCV 36        //GPIO 36 (Analog ADC1_CH0)
...
void setup() {
  ...
  CurrentSensor = Current_1;  // Change Input channel as needed
}
...
void loop() {
  ...
  SensorRAW = CalculateAverage(CurrentSensor);
  Serial.print(SensorRAW);

  // Calculating SensorZeroVoltage @  0A
  SensorZeroVoltage = ADCFactorVoltage * SensorRAW;  // Read the voltage from sensor
